...
class LoggerHelper:
    ...
    @classmethod
    def iterate(cls, iterable: Iterable[T], title: str="", total: Optional[int]=None, rate_limit: float=1.0) -> Iterable[T]:
...
class JobLogger:
    ...
    @classmethod
    def clearLogger(cls: Type[TJobLogger], job_id: Optional[str]=None) -> Optional[TLogger]:
        current_task = CurrentMessage.get_current_message()
        if job_id is None:
            if not current_task:
                return None
    ...
    def warning(self, *s, collapse: Optional[str]=None, exception: Optional[Exception]=None, send: bool=False) -> None:
        text = " ".join(str(k) for k in s)
    ...
    def progress(self, current=0, total=None, title="", key:Optional[str]=None, end=False, print: bool=False, send: bool=True, **kwargs) -> None:
        if key is None:
            key = title
    ...
    def iterate(self, iterable: Iterable[T], title: str="", total: Optional[int]=None, rate_limit:float=1.0) -> Iterable[T]:
        """
